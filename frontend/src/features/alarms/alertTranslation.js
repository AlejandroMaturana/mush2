// D3/T10 — Traducción 5 niveles de alerta (Nivel 1, frontend-only).
// Cada alarma se explica con el patrón:
//   Qué ocurrió → Qué significa → Qué impacto → Qué debería hacer → Cómo verificar
// Fuente: M0.3 T10 (Quick Win UX, sin backend). NO inventa recomendaciones de
// hardware (límite M0.1 D3). Usa únicamente los campos del modelo Alarm.
...
// Copia curada por tipo. `{sensor}` se sustituye por el sensor cuando aplica.
...
export function explainAlarm(alarm = {}) {
  ...
  const fmt = (tmpl) => {
    ...
    if (alarm.currentValue != null && !s.includes(String(alarm.currentValue))) {
      s = `${s} (${alarm.currentValue}${alarm.sensorType === 'HUMIDITY' ? '%' : alarm.sensorType === 'CO2' ? ' ppm' : alarm.sensorType === 'VOC' ? ' ppb' : '°C'})`
    }
    return s
  }
  ...
  return {
    what: fmt(t.what),
    meaning: fmt(t.meaning).replace('{sensor}', sensorLabel),
    impact: t.impact,
    action: t.action,
    verify: t.verify,
  }
...
export const FIVE_LEVEL = [
  { key: 'what', label: 'Qué ocurrió' },
  { key: 'meaning', label: 'Qué significa' },
  { key: 'impact', label: 'Qué impacto' },
  { key: 'action', label: 'Qué debería hacer' },
  { key: 'verify', label: 'Cómo verificar' },
]
